...
class HangmanGame {
    ...
    newGame(message) {
        ...
        message.channel.send(new Discord.MessageEmbed().setColor("BLUE").setDescription(this.getDescription()).addField("사용한 문자들", '\u200b').addField("하는 방법", "게임이 시작되면 랜덤으로 영어 단어가 정해져요.\n그 단어를 추측해서 스펠링에 맞는 반응을 달면 되요!")).then(e => {
            this.gameEmbed = e;
            this.waitForReaction()
        });
    }
    gameOver(win) {
        this.inGame.delete(this.channel.id);
        this.gameEmbed.edit(new Discord.MessageEmbed().setDescription((win ? "이겼어요!" : "졌네요..") + "\n\n정답은 " + this.word));
    }
    waitForReaction() {
        this.gameEmbed.awaitReactions((reaction, user) => user.id === this.user.id && reaction.emoji.name, { max: 1, time: 300000, erros: ['time']}).then(collected => {
    ...
    makeGuess(reaction) {
        ...
        if (this.inGame.has(this.channel.id)) {
            this.gameEmbed.edit(new Discord.MessageEmbed().setColor("BLUE").setDescription(this.getDescription()).addField("사용한 문자들", this.guesssed.length == 0 ? '\u200b' : this.guesssed.join(" ")).addField("하는 방법", "게임이 시작되면 랜덤으로 영어 단어가 정해져요.\n그 단어를 추측해서 스펠링에 맞는 반응을 달면 되요!"))
            this.waitForReaction();
        }
    }
    getDescription() {
